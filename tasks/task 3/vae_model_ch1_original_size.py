...
class Encoder(nn.Module):
    def __init__(self, in_channel, channel, n_res_block, n_res_channel):
        ...
        blocks = [
            # 3 -> 8
            nn.Conv2d(in_channel, channel // 8, 3, padding=1),
            nn.ReLU(inplace=True),
            # 8 -> 16
            nn.Conv2d(channel // 8, channel // 4, 1, padding=0),
            nn.ReLU(inplace=True),
            # 16 -> 32
            nn.Conv2d(channel // 4, channel // 2, 3, stride=2, padding=1),
            nn.ReLU(inplace=True),
            # channel  -->  channel * 2
            # 32 -> 64
    ...
    def __init__(
        ...
        blocks.extend(
            [
                ...
                nn.ReLU(inplace=True),
                # 32 -> 16
                nn.Conv2d(channel // 2, channel // 4, 1, padding=0),
                nn.ReLU(inplace=True),
                # 16 -> 8
                nn.Conv2d(channel // 4, channel // 8, 3, stride=1, padding=1),
                nn.ReLU(inplace=True),
                # 8 -> 3
                nn.Conv2d(channel // 8, out_channel, 3, stride=1, padding=1),
            ]
        )
