        ...
        if dist.is_primary():
            ...
            if i == 1:
                ...
                utils.save_image(
                    ...
                    f"sample_2/{str(epoch + 1).zfill(5)}_{str(i).zfill(5)}.png",
                    nrow=sample_size,
                    normalize=True,
                    value_range=(-1, 1),
                )
...
def main(args):
    ...
    loader = DataLoader(
        train_dataset, batch_size=32 // args.n_gpu, sampler=sampler, num_workers=2
    )
    ...
    for i in range(args.epoch):
        ...
        if dist.is_primary():
            torch.save(model.state_dict(), f"checkpoint/vqvae_2_{str(i + 1).zfill(3)}.pt")
...
if __name__ == "__main__":
    ...
    # download and save the CIFAR-10 dataset as an image folder
